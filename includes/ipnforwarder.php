...
class withinweb_wwkc_keycodes_ipn_forwarder {
	
	//--------------------------------------------
    //init for the Ipn_Forwarder
    public static function init() {

        add_action('withinweb_wwkc_keycodes_ipn_smarter_forwarding_handler', array(__CLASS__, 'withinweb_wwkc_keycodes_ipn_forwarding_handler'), 10, 2);
        ...
        if (isset($woocommerce_paypal_settings['enabled']) && $woocommerce_paypal_settings['enabled'] == 'yes') {
            add_filter('woocommerce_paypal_args', array(__CLASS__, 'withinweb_wwkc_keycodes_standard_parameters'), 10, 1);            
        }
        
        add_filter('withinweb_wwkc_keycodes_ipn_forwarding_setting', array(__CLASS__, 'withinweb_wwkc_keycodes_ipn_forwarding_setting'), 10, 1);
        
        add_filter('withinweb_wwkc_keycodes_ipn_forwarding_remote_post_response', array(__CLASS__, 'withinweb_wwkc_keycodes_ipn_forwarding_remote_post_response'), 10, 4);
        add_action( 'withinweb_wwkc_keycodes_ipn_forwarding_remote_post', array(__CLASS__, 'withinweb_wwkc_keycodes_ipn_forwarding_remote_post'), 10, 3 );
    }
	//--------------------------------------------
    //withinweb_wwkc_keycodes_ipn_forwarding_handler helper function used for IPN handler
    public static function withinweb_wwkc_keycodes_ipn_forwarding_handler($posted, $post_id) {

        if (!isset($posted) || empty($posted)) {
            return;
        }
        $posted = stripslashes_deep($posted);
        $ipn_forwarding_setting_serialize = apply_filters('withinweb_wwkc_keycodes_ipn_forwarding_setting', maybe_unserialize(get_option('ipn_forwarding_setting')));
            ...
            foreach ($ipn_forwarding_setting_serialize as $serialize_key => $serialize_value) {
                if ((isset($serialize_value['paypal_ipn_url']) && !empty($serialize_value['paypal_ipn_url'])) && isset($serialize_value['active_inactive_checkbox']) && $serialize_value['active_inactive_checkbox'] == 'on') {
                    if (isset($serialize_value) && count($serialize_value) <= 3) {
                        withinweb_wwkc_keycodes_ipn_forwarder::withinweb_wwkc_keycodes_ipn_forwarding_remote_post($posted, $serialize_value, $post_id);
                    } else {
                        ...
                        if (isset($paypal_ipn_field) && !empty($paypal_ipn_field)) {
                            switch ($paypal_ipn_field) {
                                ...
                                case 'transaction_type':
                                    $paypalhelper = new withinweb_wwkc_keycodes_paypal_helper();
                                    $txn_type = withinweb_wwkc_keycodes_ipn_forwarder::withinweb_wwkc_keycodes_parse_txn_type($posted);
                                    $txn_type_own = $txn_type['txn_type_own'];
                                    if (withinweb_wwkc_keycodes_ipn_forwarder::withinweb_wwkc_keycodes_ipn_forwarding_dynamic_condition($serialize_value, $posted, $txn_type_own) == true) {
                                        withinweb_wwkc_keycodes_ipn_forwarder::withinweb_wwkc_keycodes_ipn_forwarding_remote_post($posted, $serialize_value, $post_id);
                                    }
                                    break;
                                default:
                                    if (withinweb_wwkc_keycodes_ipn_forwarder::withinweb_wwkc_keycodes_ipn_forwarding_dynamic_condition($serialize_value, $posted) == true) {
                                        withinweb_wwkc_keycodes_ipn_forwarder::withinweb_wwkc_keycodes_ipn_forwarding_remote_post($posted, $serialize_value, $post_id);
                                    }
                                    break;
                            }
    ...
    public static function withinweb_wwkc_keycodes_standard_parameters($paypal_args) {
      
        $paypal_args['notify_url'] = add_query_arg( 'keys', 'paypal_standard', get_option('withinweb_wwkc_keycodes_ipn_url') );
    ...
    public static function withinweb_wwkc_keycodes_ipn_forwarding_setting($ipn_forwarding_setting) {
        if( !empty($_GET['keys']) ) {
            $inbuild_ipn_forwarding_array = array();
            if('paypal_standard' == sanitize_text_field($_GET['keys'])) {
	...
	//--------------------------------------------
    public static function withinweb_wwkc_keycodes_ipn_forwarding_remote_post($posted = null, $serialize_value = null, $post_id) {
        $params = array(
            'body' => $posted,
            'headers' => array(
        ...
        return apply_filters('withinweb_wwkc_keycodes_ipn_forwarding_remote_post_response', $response, $posted, $serialize_value, $post_id);
    }
	//--------------------------------------------
    public static function withinweb_wwkc_keycodes_ipn_forwarding_dynamic_condition($serialize_value = null, $posted = null, $txn_type_own = null) {
        $paypal_ipn_field = $serialize_value['paypal_ipn_field'];
	...
	//--------------------------------------------
    public static function withinweb_wwkc_keycodes_ipn_forwarding_dynamic_condition_for_txn_type($serialize_value = null, $posted = null, $txn_type_own) {
        $paypal_ipn_fieldvalue = $serialize_value['paypal_ipn_fieldvalue'];
        if (isset($txn_type_own) && !empty($txn_type_own)) {
	...
	//--------------------------------------------
    public static function withinweb_wwkc_keycodes_parse_txn_type($posted = null) {
    ...
    //--------------------------------------------
    public static function withinweb_wwkc_keycodes_ipn_forwarding_remote_post_response($response, $posted, $serialize_value, $post_id) {
...
withinweb_wwkc_keycodes_ipn_forwarder::init();
